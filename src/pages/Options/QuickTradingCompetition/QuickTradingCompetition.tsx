...
import QuickTradingTable from '../QuickTrading/QuickTradingTable';
...
const QuickTradingCompetitionPage: React.FC<any> = () => {
    try {
        ...
        const orders: ExtendedOrderItem[] =
            ordersQuery.isSuccess && ordersQuery.data
                ? ordersQuery.data.filter((order) => {
                      ...
                      const marketCreationDate = new Date(order.market.timestamp);
                      const marketMaturityDate = new Date(order.market.maturityDate);
                      return (
                          marketCreationDate >= marketCreationCompetition &&
                          marketMaturityDate <= marketEndingCompetition
                      );
                  })
                : [];
        const myOrders: ExtendedOrderItem[] =
            myOrdersQuery.isSuccess && myOrdersQuery.data
                ? myOrdersQuery.data.filter((order) => {
                      const marketCreationCompetition = new Date('Oct 10 2021 10:00:00 UTC');
                      const marketEndingCompetition = new Date('Nov 01 2021 11:00:00 UTC');
                      const marketCreationDate = new Date(order.market.timestamp);
                      const marketMaturityDate = new Date(order.market.maturityDate);
...
export default QuickTradingCompetitionPage;
