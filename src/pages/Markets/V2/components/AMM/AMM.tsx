...
const AMM: React.FC<AMMProps> = ({ marketAddress, longAddress, shortAddress }) => {
    ...
    const slippage = SLIPPAGE_PERCENTAGE[2];
    ...
    const fetchAmmPriceData = async (isRefresh: boolean, isSubmit = false) => {
        ...
        if (isAmountEntered && marketAddress) {
            try {
                const { ammContract } = snxJSConnector as any;
                const ammContractWithSigner = ammContract.connect((snxJSConnector as any).signer);
                ...
                let [ammQuote, ammPriceImpact]: Array<BigNumber> = await Promise.all(promises);

                if (isNonDefaultStable) {
                    ammQuote = (ammQuote as any)[0];
                }
                ...
                if (isSubmit) {
                    ...
                    if (
                        ...
                        fetchGasLimit(marketAddress, SIDE[optionSide], parsedAmount, ammQuote, parsedSlippage);
                    }
                }
    ...
    const onMaxClick = async (isBuy: boolean) => {
        ...
        if (isBuy) {
            ...
            if (calcPrice) {
                ...
                let ammQuote = await getQuoteFromAMM(
                    isNonDefaultStable,
                    isBuy,
                    ammContractWithSigner,
                    suggestedAmount,
    ...
    return (
        <Wrapper>
            ...
            <WalletBalance
                ...
                type={optionSide}
                stableIndex={isBuy ? selectedStableIndex : undefined}
            />
            <Switch
