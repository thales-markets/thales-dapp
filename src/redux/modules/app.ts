...
export const appSlice = createSlice({
    ...
    reducers: {
        ...
        fetchAppStatusSuccess: (state, action: PayloadAction<{ isSystemSuspended: boolean; isPVT: boolean }>) => {
...
export const getIsSystemSuspended = (state: RootState) =>
    !!getAppState(state).isSystemSuspended || (!!getAppState(state).isPVT && process.env.REACT_APP_IS_PROD === 'true');

export const { setAppReady, fetchAppStatusRequest, fetchAppStatusFailure, fetchAppStatusSuccess } = appSlice.actions;
...
function* fetchSystemStatus() {
    ...
    try {
        const [isSystemSuspended, isPVT] = yield all([SystemStatus.isSystemUpgrading(), DappMaintenance.isPausedSX()]);
