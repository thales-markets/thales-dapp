...
const useBinaryOptionsUserTransactionsQuery = (
    ...
    return useQuery<OptionsTransactions>(
        QUERY_KEYS.BinaryOptions.UserMarketTransactions(marketAddress, walletAddress),
        async () => {
            try {
                const response = await axios.get(
                    `${generalConfig.API_URL}/${API_ROUTES.OptionTransactions}/${networkId}?market=${marketAddress}&account=${walletAddress}`
                );
